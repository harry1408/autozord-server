...
export async function getStats(shopId: string | null) {
  const now = new Date();
  const todayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate());
  const todayEnd = new Date(todayStart.getTime() + 24 * 60 * 60 * 1000);
  const scope = shopScope(shopId);
  ...
  const [
    ...
    prisma.estimate.count({
      where: { ...scope, deletedAt: null, status: { in: ['DRAFT', 'SENT'] } },
    }),
    prisma.payment.aggregate({
      where: { ...scope, paidAt: { gte: todayStart, lt: todayEnd } },
      _sum: { amount: true },
    }),
    prisma.repairOrder.count({
      where: {
...
export async function getRevenueChart(days: number, shopId: string | null) {
  const endDate = new Date();
  const startDate = new Date(endDate.getTime() - days * 24 * 60 * 60 * 1000);

  const payments = await prisma.payment.findMany({
    where: { ...shopScope(shopId), paidAt: { gte: startDate, lte: endDate } },
...
export async function getRecentOrders(shopId: string | null) {
...
export async function getActivityFeed(shopId: string | null) {
